...
-   The relationship if @fig-TSG appears to be positive. It looks reasonably linear and I am unsure if another fit would align with the data better.
:::
...
-   It is not reasonable to assume that white poverty causes an increase in Trump support. We have not proven any causation, only proven that there is correlation. Any number of factors could explain both white poverty and Trump support. Additionally, it could be a complete coincidence.
:::
...
```{r}
six_pct_prediction <- get_regression_points(poverty_mod, newdata=tibble(poverty=6))
six_pct_prediction
```

-   At a value of 6% white poverty, I might expect `{r} round(six_pct_prediction$trump_support_hat,1)`% support for Trump.
:::
...
-   I believe that it is unwise to use this model to predict Trump support at 85% white poverty because 85% is a very extreme outlier. The highest percentage of white poverty measured is `{r} max(trump$poverty)`% and it would be unwise to assume that the linear trend continues forever.
:::
...
```{r}
ggplot(trump, aes(x=non_white, y=trump_support)) +
  geom_point() +
  labs(x="Percentage of non-white people in each region",
       y="Percentage of voters who voted for Trump",
       title="Comparison of 2016 non-white population and Trump voter support") +
  theme_bw()
```
:::
...
-   I expect that the correlation coefficient for `non-white` and `trump_support` to be negative due to the points in the plot above being higher on the y-axis when they are farther left on the x-axis.

```{r}
cor(trump$non_white, trump$trump_support) -> non_white_correlation
```
The correlation is `{r} non_white_correlation`, which is `{r} if(non_white_correlation < 0){"negative"} else {"not negative"}`.
:::
...
```{r}
lm(trump_support~non_white, data=trump) -> nw_mod
get_regression_table(nw_mod) -> nw_mod_table
kable(nw_mod_table)
```

-   The intercept is estimated to be `{r} nw_mod_table$estimate[1]`% at 0% non-white voters.

-   The slope is estimated to be `{r} nw_mod_table$estimate[2]`% for each percentage increase in non-white voters.
:::
...
```{r}
nw_70 <- predict(nw_mod, newdata=data.frame(non_white=70))
nw_70
```

-   This linear model predicts that there will be a `{r} round(nw_70[1],2)`% level of Trump support for this region.
:::
...
```{r}
nw_cor <- cor(trump$trump_support, trump$non_white)
pov_cor <- cor(trump$trump_support, trump$poverty)
trump |> summarize("Non-white correlation" = nw_cor,
                   "Poverty correlation" = pov_cor)
nw_better_expl = abs(nw_cor) > abs(pov_cor)
```
-   `{r} if(nw_better_expl){"non_white"} else{"poverty"}` seems to be a more effective explanatory variable than the other. `{r} if(nw_better_expl){"non_white"} else{"poverty"}` has a greater absolute value to its correlation coefficient, standing at `{r} if(nw_better_expl){nw_cor} else{pov_cor}`. Values closer to -1 or 1 represent a stronger correlation, so this value suggests that it's a stronger predictor.
:::
...
```{r}
predict(nw_mod, newdata=data.frame(non_white=61)) -> aoc_predict
aoc_predict
```

**A.** Representative Ocasio-Cortez would predict that **`{r} round(aoc_predict,2)`%** of voters would support Trump in that region.

```{r}
trump |>
  filter(non_white==61) |>
  select(non_white, trump_support)
```

**B.** Her prediction would be an **overestimate**. In the observed data, the only recorded area with 61% non-white people had 33% support for Trump, which is **`{r} abs(round(33-aoc_predict,2))`% higher** than anticipated.

**C.** In other words, the residual for this prediction is **`{r} round(33-aoc_predict,2)`%**.
:::
